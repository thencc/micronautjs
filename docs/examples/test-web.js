import MicronautJS from "../dist/index.js";
...
async function runTests() {
  ...
  const escrow = Micronaut.getAppEscrowAccount(49803676);

  console.log(escrow);
  // LSJOD5C26OTW5VZIG4KF43UTU4UZY7LGXTGWMW366ZDBSFND4MZDX66HHI

  const accountInfo = await Micronaut.getAccountInfo(
    "LSJOD5C26OTW5VZIG4KF43UTU4UZY7LGXTGWMW366ZDBSFND4MZDX66HHI"
  );
  console.log("account info is", accountInfo);

  // call bricks oracle on test net: 49584323
  const bricksInfo = await Micronaut.getAppInfo(49584323);

  bricksInfo.globals.forEach((kv) => {
    if (kv.key == "bricks_per_algo") console.log(kv.value);
  });
}
...
function testWCConnect(event) {
  ...
  Micronaut.connectAlgoWallet(wcListner);
}

async function testWCTransaction() {
  console.log("testing WC Transaction");

  const txns = await Micronaut.createWalletConnectTransactions([
    await Micronaut.atomicOptInASA(49528267),
  ]);

  const res = await Micronaut.sendWalletConnectTxns(txns);
}
